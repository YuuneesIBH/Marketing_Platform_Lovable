...
const Team = () => {
  ...
  const handleCreate = () => {
    ...
    addMember({ name: form.name, role: form.role, email: form.email, initials, color: "" });
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button><Plus className="w-4 h-4" /> Lid toevoegen</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4 mt-2">
              <div><Label>Naam</Label><Input placeholder="Bijv. Anna de Jong" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
              <div><Label>Rol</Label><Input placeholder="Bijv. Content Creator" value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} /></div>
              <div><Label>E-mail</Label><Input type="email" placeholder="anna@oppaseoul.nl" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {team.map((m, i) => (
          <div key={`${m.name}-${i}`} className="glass-card rounded-xl p-6 animate-fade-in group relative" style={{ animationDelay: `${i * 80}ms` }}>
            <button onClick={() => handleDelete(i)} className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-md hover:bg-destructive/10 text-muted-foreground hover:text-destructive">
